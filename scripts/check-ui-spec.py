...
def actions(document=None):
    ...
    for op in catalog:
        ...
        if phase == 25 and op['id'] in ('integration.configureWebhook', 'integration.rotateSigningKey',
                                      'integration.getDeliveryStatus', 'integration.getDeliveryDetail',
        ...
        if phase in (28, 29, 32, 33, 34, 35) and family == 'local-ui': implemented_scope = True
        if phase == 37 and op['id'] in ('report.requestExcelExport', 'report.getExportStatus', 'report.downloadExport'): implemented_scope = True
        if phase == 38 and family == 'diagnostics': implemented_scope = True
        ...
        if not implemented_scope:
            require(op['lifecycle'] == 'designed', f'Operation outside verified workspace/implemented phases through P38 scope promoted: {op["id"]}')
    print(f'PASS B: {len(rows)} action/effect rows cover all {len(known)} canonical operations; role/state/surface/phase/requirements present. Current catalog lifecycles checked; no UI completion inferred.')
