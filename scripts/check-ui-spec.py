...
def actions(document=None):
    ...
    for op in catalog:
        ...
        implemented_scope = (
            ...
            or (phase in (15, 16, 17, 18) and family == 'execution')
        )
        if not implemented_scope:
            require(op['lifecycle'] == 'designed', f'Operation outside verified workspace/P07–P18 scope promoted: {op["id"]}')
    print(f'PASS B: {len(rows)} action/effect rows cover all {len(known)} canonical operations; role/state/surface/phase/requirements present. Verified workspace/P07–P18 scope only; no UI completion inferred.')
