...
def actions(document=None):
    ...
    for op in catalog:
        ...
        if not implemented_scope:
            require(op['lifecycle'] == 'designed', f'Operation outside verified workspace/P07–P25 scope promoted: {op["id"]}')
    print(f'PASS B: {len(rows)} action/effect rows cover all {len(known)} canonical operations; role/state/surface/phase/requirements present. Verified workspace/P07–P25 scope only; no UI completion inferred.')
