...
export async function generateArtifacts(bundle) {
  ...
  const types = '// Generated by npm run contracts:generate. P07–P27 sessions, provisioning, intake, locations, routing metadata, durable planning online round start and explicit current activity; P34 replay and P35 account recovery/update compatibility are locally verified; remaining operations follow their catalog lifecycle.\n' + astToString(await openapiTS(pathToFileURL(resolve(root, 'contracts/openapi.yaml')), { alphabetize: true }));
  const lines = [
