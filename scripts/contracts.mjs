...
export async function generateArtifacts(bundle) {
  ...
  const types = '// Generated by npm run contracts:generate. P07–P27 sessions, provisioning, intake, locations, routing metadata, durable planning online round start and explicit current activity; P34 ordered replay/conflict recovery is locally verified; remaining operations follow their catalog lifecycle.\n' + astToString(await openapiTS(pathToFileURL(resolve(root, 'contracts/openapi.yaml')), { alphabetize: true }));
  const lines = [
