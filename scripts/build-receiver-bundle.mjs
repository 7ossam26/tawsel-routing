...
for(const [source,name] of [['packages/api-client','api-client'],['apps/mock-erp','mock-erp']]){
 ...
 if(name==='mock-erp'){await cp(`${source}/migrations`,`${target}/${name}/migrations`,{recursive:true});await cp(`${source}/config.example.json`,`${target}/${name}/config.example.json`);}
}
...
await writeFile(`${target}/README.md`,guide);
for(const [source,name] of [['docs/erp/receiver-protocol.md','receiver-protocol.md'],['docs/verification/integration.md','integration-evidence.md'],['docs/phase-26-evidence.md','phase-26-evidence.md'],['docs/provisioning.md','provisioning.md'],['docs/outbox-delivery.md','outbox-delivery.md']]){
