...
test('explicit select → heading → arrival through real API, delayed plan, contact separation and lost response recovery',async({page,context,request})=>{
 ...
 for(const name of [/اتصال بـ/,/واتساب مع/,/الاتجاهات إلى/])await page.getByRole('link',{name}).click();
 ...
 await page.getByRole('list',{name:'قائمة المحطات المتاحة'}).getByRole('button',{name:/حسام الدين/}).click();expect((await (await request.get(fixture+'/__fixture/state')).json()).history).toHaveLength(0);
 ...
 const other=await context.browser()!.newContext({baseURL:'http://localhost:5178',locale:'ar-EG',viewport:{width:390,height:844}}),otherPage=await other.newPage();await otherPage.goto('/rounds/current?kind=personal');await expect(otherPage.getByText('الجولة تعمل على هاتف آخر',{exact:true})).toBeVisible();await expect(otherPage.getByRole('button',{name:'وصلت',exact:true})).toBeDisabled();await other.close();
 ...
 await page.getByRole('button',{name:'وصلت',exact:true}).tap();await expect(page.getByText('إجراء ينتظر التأكيد',{exact:true})).toBeVisible();await expect(page.getByRole('button',{name:'تحقّق وأعد إرسال الطلب نفسه'})).toBeEnabled();
 ...
 await page.unroute('**/api/v1/current/arrival?kind=personal');await page.reload();await expect(page.getByText(/الوصول مسجّل/)).toBeVisible();await expect(page.getByText('إجراء ينتظر التأكيد',{exact:true})).toHaveCount(0);await expect(page.getByRole('button',{name:'وصلت',exact:true})).toHaveCount(0);
