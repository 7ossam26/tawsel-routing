...
The [final requirement ledger](docs/verification/requirement-ledger.md) and [Phase 42 evidence](docs/phase-42-evidence.md) give the current code/test/artifact state. Historical phase evidence records actual schema, PostgreSQL, HTTP, OIDC, browser/PWA, reference ERP, reporting, performance and recovery checks under their stated conditions. No local result implies target or owner acceptance; remaining conditions are in [A–P readiness](docs/verification/pilot-readiness.md).

The initial repository assessment found Engine configuration and nine static Stitch exports. [Repository/UI assessment](docs/planning/repository-and-ui-assessment.md) records those sources; all 18 original HTML/image hashes remain checked. Prototype timers/alerts do not prove runtime behavior. `STACK-CONTEXT.md` retains historical measurements, not fresh target-capacity evidence.
...
These are required operation families. Actual available URL/field names and their local verification lifecycle are in the canonical OpenAPI and operation catalog; the final handoff identifies their exact package digests.
...
The ERP handoff includes docs/erp/README.md, ERP-PLANNING-INPUT.md, field-and-status-mapping.md, consumer-quickstart.md and release-manifest.json alongside docs/ERP-INTEGRATION-HANDOFF.md and the canonical contracts/examples/public client/reference mock/conformance suite. These are versioned local handoff outputs with exact ownership and proof criteria in the handoff deliverables plan; their final verification is recorded in Phase 42 evidence. The real ERP calls Tawsel's published boundary; Tawsel privately calls the Engine. P26/P27 must already establish external interoperability before P42 assembles the final bundle; files alone cannot establish completion.
