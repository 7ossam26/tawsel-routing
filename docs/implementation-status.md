...
The next execution prompt is [Phase 06 — Tenant, branch and capability enforcement](phases/06-tenant-capabilities-isolation.md). It may rely on the P05 migration/pool/transaction kernel, scoped command result/audit/outbox structures, and real PostgreSQL test harness documented in [P05 evidence](phase-05-evidence.md). It must add genuine tenant/resource/authorization assertions. Business HTTP operations remain designed; no Phase 06 work was performed.
