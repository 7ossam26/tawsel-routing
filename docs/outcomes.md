...
[Explicit whole retry](eligibility.md) creates a new admitted attempt while preserving every old outcome and fee. `OutcomeSnapshot.history` contains prior attempts and, since P23, original and corrected report revisions. `items`/processed quantities use the effective revision of each latest resolved attempt; collection totals include one effective report per attempt, retaining prior fees without summing superseded reports. Rejected partial remainders stay return-required and non-retryable. [P23 correction/adoption rules and demonstration](corrections.md) supersede the earlier future-correction statements. P17/P18 sections above retain their dated evidence.
