...
## Phase 31 driver surfaces

`/execution/branch?kind=company` displays source-selected offers, fixed visit claims, all retained customer positions and explicit arrival/resume. `/rounds/current` replaces customer movement controls with the branch stage while interrupted; ownership transfer retains this same segment. Arrived customer blockers link back to the current customer. Request, arrival and native actual receipt are separate. [Reproducible connected checks](phase-31-evidence.md).
