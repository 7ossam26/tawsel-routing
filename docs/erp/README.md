...
P27 (24 September 2026): [source protocol](source-protocol.md), [complete two-way setup/checkers](consumer-quickstart.md), [worked mappings](field-and-status-mapping.md), [ordered ERP implementation input](ERP-PLANNING-INPUT.md) and [checkpoint evidence](../phase-27-evidence.md) are locally verified. Runtime artifacts: `apps/mock-erp/dist`, `apps/mock-erp/ui-dist`, `apps/mock-erp/migrations/0003_source.sql`, `0004_native_sessions.sql`, `packages/api-client/dist/source.js`, and portable `dist/erp-reference/conformance/source.mjs` plus `receiver.mjs`. Source implementation and tests live in `apps/mock-erp/src/source.ts`, `native.ts`, `native-auth.ts`, `ui/`, `tests/source-browser/` and `tests/erp-conformance/source.ts`. Run `npm run source:demo` for independent install/process/recovery proof. Native staff and driver have separate actual OIDC sessions; source acceptance, outbound receipt and receiver application are separate. Historical updates below retain their original phase bounds.

P26 (24 September 2026): the [independent durable receiver](receiver-protocol.md), [standalone install/start/conformance](consumer-quickstart.md), [real process/database proof](../verification/integration.md) and [ordered evidence](../phase-26-evidence.md) are available. Build artifacts: packages/api-client/dist, apps/mock-erp/dist and dist/erp-reference. Received, applied and snapshot-covered state remain distinct, with explicit historical gaps. P27 source delivery is now verified above; real vendor ERP integration remains separate. Earlier numbered summaries below describe their historical phase scope.
...
2. [field-and-status-mapping.md](field-and-status-mapping.md): verified identity/source/shipment/intake and actual subset receipt/disposition mapping, canonical fields and verified redispatch/correction mappings.
...
| P08 source/branch/role/user/driver provisioning | 12 commands, two reads, explicit service authentication, source revisions and issuer reconciliation verified locally | Production issuer ownership/permissions; native administration is P27 |
| Public client and consumer quickstart/conformance | Compiled independent bundle and P27 source/receiver two-way conformance verified; no Tawsel internals or database credentials | Real vendor connector and P42 release packaging |
...
| Execution and reports | P15–24 public execution and scoped monitoring verified; P27 public driver journey | Full driver/reporting screens in later phases |
| Branch receipt/disposition/redispatch | P21/P22 APIs and P27 native physical subset/new cycle verified | Commercial ERP stock semantics remain vendor-owned |
| provisioning.changed event | Atomic source-scoped intent, signed delivery and independent application verified through P25–27 | Production key and receiver operations |
| Full signed delivery/independent receiver/source | P25–27 locally verified; independent bundle and integration evidence available | Production/real ERP conformance and P42 release |
| Final docs/ERP-INTEGRATION-HANDOFF.md and release-manifest.json | Not created; no release identity/digests invented | P42 |

Use npm run contracts:check, npm run test:contracts and the public client typecheck for canonical conformance; npm run test:provisioning for actual isolated PostgreSQL/provider-fixture proof; npm run test:erp:provisioning for running public HTTP; npm run test:browser:provisioning for actual local Keycloak/Chromium. Their setup and effects are in the quickstart. Legacy provisioning CLI journals are not transactional; the P27 source CLI commits source rows and outbox together, and local email/localhost is not production SMTP/TLS evidence.
