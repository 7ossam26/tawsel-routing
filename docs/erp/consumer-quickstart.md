...
Phase 40 reproduction: run `npm run test:backup` and `npm run backup:rehearse` with the [documented local Windows prerequisites](../recovery.md#run-the-local-rehearsal). The rehearsal invokes the existing public receiver conformance checker after restoring real databases and issuer configuration. This is a synthetic local checkpoint, not a restore of your ERP. Use your independently protected inbox/source-outbox checkpoint and secrets, then compare received/applied/snapshot state through the existing public interfaces; never erase receipt history to recover. [Timed evidence](../verification/restore.md).

Phase 39 managed release: set `MOCK_ERP_MANAGED_MIGRATIONS=true`, run the existing `main.js migrate` once before starting the receiver, projection worker or source worker. Their startup then checks exact migration history and recipient scope without changing schema. A missing/changed migration fails startup; never reset the inbox/source outbox to recover. The [private mock deployment section](../deployment.md#private-labelled-mock) distinguishes loopback-only staging from unverified target connectivity. Public consumer contracts/client/conformance commands are unchanged; [actual release checks](../verification/deployment.md) include real separate-database receiver/readiness proof.

Phase 35 consumer checks: run `npm run recovery:demo` with the existing local PostgreSQL/Keycloak prerequisites, then `npm run test:erp:recovery -- .local/phase-35-recovery-browser.json .local/phase-35-retained-review.json`. The public-only checker verifies unchanged original envelopes, received/accepted versus received/review-required receipts, native migration rollback evidence, and unresolved server evidence after permitted logout. It imports no Tawsel database or internal service. Without reports, `npm run test:erp:recovery` checks only the labelled canonical expected-account recovery example. This is local interoperability evidence, not ERP application or physical-device certification. Human recovery uses exported `SessionClient` with session cookies/CSRF; ERP service tokens gain no driver authority. [Protocol/reproduction](../offline-account-updates.md), [actual results and gaps](../phase-35-evidence.md).

Phase 34 consumer checks: `npm run test:erp:sync` checks canonical per-entry/waiting/receipt semantics only. Run `npm run replay:demo` against existing local PostgreSQL/Keycloak, then `npm run test:erp:sync -- .local/phase-34-reconnect-evidence.json .local/phase-34-two-device-evidence.json` to check actual public receipts, unchanged original envelopes and separate adopted outcome references from the two browser scenarios. The checker reads no Tawsel database or internal service; the operator demo harness owns isolated setup and assertions. These sync/adoption endpoints require a human driver session and CSRF, not ERP service credentials. Accepted outcomes still travel through the existing signed outbox/receiver boundary; the demo does not assert commercial ERP application. [Reproduction and Phase 35 boundary](../ordered-replay.md), [exact results](../phase-34-evidence.md).

Phase 33 consumer check: `npm run test:erp:planning` now checks the optional `PlanningRoutePolicy.roadRoute` example while older/manual plans continue to work without geometry. No ERP endpoint/event is added. Consumers must not treat browser-local saved work as server acceptance or ERP application; the [local format](../../contracts/local-work.schema.json) is not a transport contract. `npm run test:browser:offline` reproduces production-PWA download/disconnect/reopen and verifies unsent IDs are absent from a real authorized monitoring read; this is local Chromium/Keycloak/PostgreSQL evidence with controlled routing, not commercial ERP or physical-phone proof. [Exact instructions and limits](../offline-local-capture.md).
...
## Phase 30 exception/correction verification

Run `npm run test:browser:exceptions` with the existing local PostgreSQL/Keycloak prerequisites. It creates disposable company/personal users and an isolated application database, uses public mock source intake plus real driver routes, and verifies partial/refusal/unpaid fees, future deferral, retry/capacity, assigned-driver urgency/pin and accepted/receipt/day-denied correction. Routing is controlled HTTP; no production Engine/commercial ERP claim. Then run `node --import tsx tests/erp-conformance/exceptions.ts .local/phase-30-browser-evidence.json` to check captured public JSON. Without the report argument, that checker validates canonical examples/negative controls only. No database access is used by the consumer checker. [Tap flow, screenshots and limitations](../phase-30-evidence.md).

For correction ownership reads use `executionRoundId` (fall back to `roundId` for older captures); send the original `roundId` in the correction payload. The P30 public-response check verifies that current reads supply this anchor.
...
## Phase 36 reporting capture

With the existing local PostgreSQL and Keycloak services running and supported Node 24, run npm run test:reporting, npm run reporting:demo, then npm run test:erp:reporting -- .local/phase-36-browser-evidence.json. The demo uses disposable users/database, actual HTTP/OIDC/browser interaction, and an explicitly controlled HTTP routing provider through the real planning worker. It produces accepted report snapshots before/after correction and an authorized shared-round staff view, plus 403/404 isolation evidence. Screenshots supplement interaction assertions.

The conformance module consumes only public JSON/types; it needs no database/server/domain imports or issuer credentials. It checks denominators, separate currencies, piece conservation, missing-arrival duration rules, original correction timing, changed workload, unfinished closure and authorization statuses. This is local capture interoperability evidence, not a commercial ERP integration claim. Reports are human session APIs: do not reuse integration bearer credentials as driver/staff authentication.

Use [the single report query and snapshot definitions](../reporting.md#exact-phase-37-handoff) for the next export phase. Export operations remain designed; no Excel generation/download is released here.

## Phase 37 authorized Excel export
