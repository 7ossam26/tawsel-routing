...
P35 update — 25 September 2026: [safe account recovery and update compatibility](../offline-account-updates.md) now preserve original queued evidence across same-account reauthentication, explicit exit, atomic local-schema upgrade and waiting-worker activation. Received review permits exit while server evidence remains unresolved. [Ordered results and honest device limits](../phase-35-evidence.md). Phase 36 is next and may rely on the [exact verified artifacts](../offline-account-updates.md#exact-phase-36-handoff); it was not executed here. Earlier updates retain their historical scope.
...
P25 update — 24 September 2026: durable scoped signed sender, recovery/replay and operational reads are implemented and locally verified. [Protocol/demo](../outbox-delivery.md), [evidence](../phase-25-evidence.md). Phase 26 is next and owns the separate durable receiver/application proof. Earlier phase summaries below are historical.

P24 update — 24 September 2026: coherent conditional monitoring and scoped driver/trip/task/workday/action history are implemented and locally verified. See [evidence](../phase-24-evidence.md) and [API/demo](../monitoring.md). Phase 25 is next; signed delivery, native ERP application, polling UI and measured freshness remain later work.
...
التنفيذ الفعلي: **المراحل 01–27 منفذة ضمن حدود كل مرحلة؛ المرحلة 09 تضيف إدخال مهام الحساب المستقل بقاعدة PostgreSQL وواجهة عربية واختبار Chromium حقيقي، والمرحلة 10 تضيف استقبال شحنات ERP والاستلام والقبول الذري؛ المرحلة 11 منفذة محليًا ضمن حدود [دليل المواقع](../locations-and-maps.md)؛ المرحلة 12 نُفذت بمحولات مختبرة ببيانات محكومة مع عدم توفر Engine الحي؛ المرحلة 13 أضافت مهام تخطيط دائمة وتوقعات محفوظة مع اختبارات PostgreSQL وإعادة التشغيل؛ المرحلة 14 أضافت سياسة العاجل والتحقق الكامل والترتيب اليدوي مع توقعات محفوظة؛ المرحلة 15 أضافت بدء الجولة والملكية وقفل المغادرة والاستلام أثناء الجولة؛ المرحلة 16 أضافت الاتجاه والوصول الصريحين ونقطة الانطلاق مع واجهة مصغّرة متصلة؛ المرحلة 17 أضافت نتائج التسليم والقطع الصحيحة والتحصيل الدقيق وتقدمًا ذريًا مع قصد إرسال دائم؛ المرحلة 18 أضافت التأجيل وإعادة المحاولة الكاملة وأولوية المندوب مع حفظ المحاولات والتحصيل؛ المرحلة 19 أضافت إنهاء الجولة واليوم الصريحين وترحيل العمل المحتفظ به وملخصًا أساسيًا دون إثبات استلام أو تسوية؛ المرحلة 20 أضافت نقل التنفيذ الصريح بين هواتف نفس المندوب مع تحميل الحالة المؤكدة وحفظ دليل الهاتف السابق دون تطبيقه؛ المرحلة 21 أضافت طلب المرتجع وتأكيد الاستلام الجزئي الفعلي والتصرف المنفصل للفقد والتلف؛ المرحلة 22 أضافت زيارة الفرع واستئناف التسلسل المحفوظ ودورات إرسال جديدة من الاستلام الفعلي؛ المرحلة 23 أضافت التصحيح المحدود واعتماد دليل الهاتف السابق مع حفظ التاريخ؛ المرحلة 24 أضافت لقطات مراقبة متسقة وتاريخًا حسب الصلاحيات مع مراجعات وقراءات شرطية؛ المرحلة 25 أضافت الإرسال الدائم الموقّع وإعادة المحاولة والاستعادة وقراءات الحالة؛ المرحلة 27 أضافت واجهة ERP تجريبية وجلسة مستقلة وصندوق إرسال دائم وإثبات تكامل في الاتجاهين؛ المرحلة 28 هي التالية**. العنوان المكتوب يحتاج تأكيد نقطة التوصيل؛ خرائط القاهرة الحقيقية متاحة محليًا وخدمة Nominatim الخاصة غير متاحة حاليًا. بقية عمليات التشغيل ما زالت مصممة فقط. راجع [سجل التنفيذ](../implementation-status.md)، [دليل المرحلة 11](../phase-11-evidence.md) و[طريقة تجربة الإدخال](../b2c-intake.md). هذه ملفات التكليف بالتنفيذ.
