...
## Phase 31 closure page

`/execution/closure?kind=personal|company` reads the day summary and current held records. `RoundSummary.activityRevision` is additive and supplied by the current server; between-round End day uses it with active=null and the latest round's real owner generation/token. The UI refuses to guess a missing revision. Workday identity remains in the URL after closure so a reload can recover the same exact action.

End round retains an open day; End day retains unfinished/held tasks for the next explicit day. Heading pause is an explicit checkbox; arrived/branch states link to the required resolution. Accepted counts and reported collection are separate from held work and settlement. Known unresolved commands on other execution pages block closure and new-round start within the current session. This is bounded online recovery, not a durable offline queue. [Actual checks](phase-31-evidence.md).
