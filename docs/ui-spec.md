...
Phase 36 implementation update — 25 September 2026: /reports and /reports/workdays/:workdayId are connected online session screens. Shared Cairo/navy cards show processed/full/partial/unfinished units before expandable round/attempt/history detail. Personal optional filters and collection stay simple; company views add branch/driver selection and piece/fee detail. Missing/uncertain time, workload change and unfinished closure use brief Arabic explanations. Keyboard disclosures, responsive cards and current-access denial are covered by [local browser evidence](phase-36-evidence.md). Export remains P37; the older specification below retains its historical/design context.
