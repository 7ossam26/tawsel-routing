...
Receipt/disposition atomically commits balances, item revision, append-only transition, `retry_dependencies`, current-custody projection, planning invalidation/job, audit, action result and source-only event intent. The original outcome is never rewritten. `OutcomeSnapshot.custody` exposes the current per-line read; `progress.heldReturnRequiredPieces` and workday carry-forward use it, while historical outcome lines retain their original return-required facts. [P23 corrections](corrections.md) now observe the same dependencies and locks. An unreceived, unclaimed offer becomes superseded if its report is corrected; actual receipt/disposition or an active claimed handover prevents correction.
