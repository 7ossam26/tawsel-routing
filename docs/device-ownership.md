...
`adoptionImplemented:false` is always explicit. `requires-validation` means only that the current metadata found no listed blocker; it never authorizes application. Designed `DeviceAdoptionCommand` references the received action/receipt and expected owner/activity/outcome/source/assignment/pin revisions. P23 must implement compatible, current-driver-only correction under the shared invariant locks and rebuild effective state atomically; P34 supplies replay/display. No arbitrary staff override or adoption HTTP route exists here.
