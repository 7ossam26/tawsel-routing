...
P41 [pilot readiness](verification/pilot-readiness.md) and [owner walkthrough](verification/pilot-walkthrough.md) join existing intake, location, planning, execution, closure, reporting/export and mock ERP operations. Local browser evidence, controlled routing, physical devices and owner approval remain separate. No public interface or operation availability is changed by this review.

**P07–P27 session/context, provisioning, intake, locations, routing metadata, durable planning, online start, current/arrival, exact outcomes, explicit eligibility transitions and workday closure/carry-forward are implemented locally; each row records its actual lifecycle.** P20 online takeover, snapshot fencing and retained evidence are locally verified; P21 source-branch return receipt/disposition and public consumer proof are locally verified; P22 branch/new-cycle dependencies and P23 correction/adoption are locally verified; P25 signed delivery/status/retry/replay are locally verified; P26 external application and P27 native source durability/two-way conformance are locally verified. [Identity setup/evidence](identity.md) identifies application HTTP paths versus issuer-hosted actions. P34 ordered replay, conflict reads and manual retry are locally verified; [protocol](ordered-replay.md) and [evidence](phase-34-evidence.md) define their limits. P35 safe reauthentication, account exit and updates are locally verified; [compatibility](offline-account-updates.md) and [evidence](phase-35-evidence.md) define their limits. No generic CRUD endpoint replaces explicit actions.
...
| `sync.submitActions` | http-command | verified-local | [P34](phases/34-ordered-replay-conflict-recovery.md) | execution.own | Dependency-ordered replay batch with per-action result, original identity/version. |
...
| `sync.listConflicts` | http-read | verified-local | [P34](phases/34-ordered-replay-conflict-recovery.md) | execution.own | Read preserved conflicts and currently permitted resolution choices. |
...
| `report.requestExcelExport` | http-command | verified-local | [P37](phases/37-authorized-excel-export.md) | reports.export | Create authorized export bound to same filter/snapshot/timezone as report. |
| `report.getExportStatus` | http-read | verified-local | [P37](phases/37-authorized-excel-export.md) | reports.export | Reauthorize and read ready/expired synchronous export status. |
| `report.downloadExport` | http-read | verified-local | [P37](phases/37-authorized-excel-export.md) | reports.export | Reauthorize and download text-safe Excel artifact before expiry. |
...
| `ui.reauthenticateSameAccount` | local-ui | verified-local | [P35](phases/35-offline-auth-updates-ux.md) | local | Preserve queue and invoke session login for its owning account. |
| `ui.switchAccount` | local-ui | verified-local | [P35](phases/35-offline-auth-updates-ux.md) | local | Block unsynchronized exit; otherwise use safe logout/login, no identity merge. |
| `ui.applySafeUpdate` | local-ui | verified-local | [P35](phases/35-offline-auth-updates-ux.md) | local | Apply versioned shell/storage update only at safe pending-work boundary. |
