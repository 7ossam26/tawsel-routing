...
describe('real PostgreSQL migration lifecycle', () => {
  ...
  test('applies fresh SQL once, serializes concurrent runners, verifies checksums', async () => {
    ...
    expect(tables.rows.map(r => r.tablename)).toEqual([
      'accounts', 'auth_rate_limits', 'b2b_assignment_history', 'b2b_dispatch_cycles', 'b2b_source_lines', 'b2b_source_snapshots', 'b2b_tasks', 'b2c_tasks', 'branch_activities', 'branch_activity_history', 'branches', 'capabilities', 'closure_records', 'command_audit', 'command_evidence', 'command_identities', 'command_replay_metadata', 'command_sources', 'company_login_codes',
      ...
      'location_history', 'location_planning_inputs', 'login_attempts', 'membership_branches', 'memberships', 'monitoring_change_marks', 'monitoring_views', 'outbox_consumer_checkpoints', 'outbox_deliveries', 'outbox_delivery_attempts', 'outbox_endpoints', 'outbox_intents', 'outbox_projection_snapshots', 'outbox_signing_keys', 'outbox_streams', 'outcome_collections', 'outcome_corrections', 'outcome_quantities', 'physical_origin_history', 'plan_revisions', 'planning_attempts', 'planning_job_attempts', 'planning_jobs', 'planning_states', 'provisioning_records', 'provisioning_sources', 'provisioning_subject_grants', 'redispatch_allocations', 'retry_dependencies', 'return_balances', 'return_items', 'return_requests', 'return_transitions', 'role_capabilities', 'roles', 'round_activity_state', 'round_admissions', 'round_publications', 'rounds', 'schema_migrations', 'service_credentials',
      'start_readiness', 'task_collection_amounts', 'task_eligibility_history', 'task_execution_options', 'task_intake_events', 'task_locations', 'task_source_addresses', 'tenant_keys', 'tenants', 'user_capability_exceptions', 'web_sessions', 'workdays', 'worker_observations'
    ]);
