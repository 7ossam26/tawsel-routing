...
describe('real PostgreSQL migration lifecycle', () => {
  ...
  test('applies fresh SQL once, serializes concurrent runners, verifies checksums', async () => {
    ...
    expect(results.flat()).toEqual(['0001_command_foundation.sql', '0002_tenant_access.sql', '0003_identity_sessions.sql', '0004_erp_provisioning.sql', '0005_provisioning_subject_scope.sql']);
    ...
    expect(tables.rows.map(r => r.tablename)).toEqual([
      ...
      'drivers', 'identity_subjects', 'integration_branches', 'integration_capabilities', 'integrations', 'issuer_reconciliation',
  ...
  test('upgrades real P05 data without rewriting commands or granting legacy keys access', async () => {
    ...
    try {
      ...
      expect(await migrate(old.pool)).toEqual(['0002_tenant_access.sql', '0003_identity_sessions.sql', '0004_erp_provisioning.sql', '0005_provisioning_subject_scope.sql']);
