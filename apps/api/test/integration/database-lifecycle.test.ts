...
describe('real PostgreSQL migration lifecycle', () => {
  ...
  test('applies fresh SQL once, serializes concurrent runners, verifies checksums', async () => {
    ...
    expect(tables.rows.map(r => r.tablename)).toEqual([
      ...
      'current_activity_history', 'driver_planned_stops', 'drivers', 'execution_attempts', 'forecast_members', 'forecast_revisions', 'identity_subjects', 'intake_replan_intents', 'integration_branches', 'integration_capabilities', 'integrations', 'issuer_reconciliation',
      'location_history', 'location_planning_inputs', 'login_attempts', 'membership_branches', 'memberships', 'outbox_intents', 'physical_origin_history', 'plan_revisions', 'planning_attempts', 'planning_job_attempts', 'planning_jobs', 'planning_states', 'provisioning_records', 'provisioning_sources', 'provisioning_subject_grants', 'role_capabilities', 'roles', 'round_activity_state', 'round_admissions', 'round_publications', 'rounds', 'schema_migrations', 'service_credentials',
