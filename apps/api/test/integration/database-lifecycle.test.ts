...
describe('real PostgreSQL migration lifecycle', () => {
  ...
  test('applies fresh SQL once, serializes concurrent runners, verifies checksums', async () => {
    ...
    expect(tables.rows.map(r => r.tablename)).toEqual([
      'accounts', 'auth_rate_limits', 'b2b_assignment_history', 'b2b_dispatch_cycles', 'b2b_source_lines', 'b2b_source_snapshots', 'b2b_tasks', 'b2c_tasks', 'branches', 'capabilities', 'closure_records', 'command_audit', 'command_evidence', 'command_identities', 'command_sources', 'company_login_codes',
      'current_activity_history', 'delivery_outcomes', 'driver_planned_stops', 'drivers', 'effective_task_outcomes', 'execution_attempts', 'forecast_members', 'forecast_revisions', 'identity_subjects', 'intake_replan_intents', 'integration_branches', 'integration_capabilities', 'integrations', 'issuer_reconciliation',
      'location_history', 'location_planning_inputs', 'login_attempts', 'membership_branches', 'memberships', 'outbox_intents', 'outcome_collections', 'outcome_quantities', 'physical_origin_history', 'plan_revisions', 'planning_attempts', 'planning_job_attempts', 'planning_jobs', 'planning_states', 'provisioning_records', 'provisioning_sources', 'provisioning_subject_grants', 'retry_dependencies', 'role_capabilities', 'roles', 'round_activity_state', 'round_admissions', 'round_publications', 'rounds', 'schema_migrations', 'service_credentials',
