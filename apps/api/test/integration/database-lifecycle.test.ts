...
describe('real PostgreSQL migration lifecycle', () => {
  ...
  test('applies fresh SQL once, serializes concurrent runners, verifies checksums', async () => {
    ...
    expect(tables.rows.map(r => r.tablename)).toEqual([
      ...
      'driver_planned_stops', 'drivers', 'forecast_members', 'forecast_revisions', 'identity_subjects', 'intake_replan_intents', 'integration_branches', 'integration_capabilities', 'integrations', 'issuer_reconciliation',
      'location_history', 'location_planning_inputs', 'login_attempts', 'membership_branches', 'memberships', 'outbox_intents', 'plan_revisions', 'planning_attempts', 'planning_job_attempts', 'planning_jobs', 'planning_states', 'provisioning_records', 'provisioning_sources', 'provisioning_subject_grants', 'role_capabilities', 'roles', 'round_admissions', 'round_publications', 'rounds', 'schema_migrations', 'service_credentials',
      'start_readiness', 'task_collection_amounts', 'task_intake_events', 'task_locations', 'task_source_addresses', 'tenant_keys', 'tenants', 'user_capability_exceptions', 'web_sessions', 'workdays'
    ]);
  ...
  test('upgrades real P05 data without rewriting commands or granting legacy keys access', async () => {
    ...
    try {
      ...
      expect(await migrate(old.pool)).toEqual(['0002_tenant_access.sql', '0003_identity_sessions.sql', '0004_erp_provisioning.sql', '0005_provisioning_subject_scope.sql', '0006_b2c_intake.sql', '0007_b2b_intake.sql', '0008_locations.sql', '0009_planning.sql', '0010_route_policy.sql', '0011_manual_plans.sql', '0012_round_start.sql']);
