...
describe('P17 outcome / progress / outbox — isolated real PostgreSQL and API',()=>{
 ...
 test('B: partial resolves arrived current atomically; preserves origin, baseline and held remainder',async()=>{
  ...
  expect(before.targets.find(t=>t.taskId===f.tasks[0])?.delivery).toMatchObject({kind:'company',allowedActions:['full','partial','refusal','no-answer'],fullCollection:money(35000),goodsDue:money(30000),shippingDue:money(5000),lines:[{sourceLineId:'pieces',description:'قطع',quantity:3,unitDue:money(10000)}]});
 ...
 test('C: B2C rejects piece/shipping workflows and supports optional exact collection without custody',async()=>{
  ...
  expect(target.delivery).toEqual({kind:'personal',allowedActions:['full','refusal','no-answer'],fullCollection:money(12500),goodsDue:null,shippingDue:null,lines:[]});
