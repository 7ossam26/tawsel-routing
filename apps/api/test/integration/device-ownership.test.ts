...
test('C: additive migration preserves an actual P19 started round and enables takeover',async()=>{
 ...
 try{
  ...
  expect(await migrate(old.pool)).toEqual(['0017_device_takeover.sql','0018_account_evidence_notifications.sql']);expect((await old.pool.query('SELECT to_jsonb(r) r FROM tawsel.rounds r')).rows).toEqual(before);
