...
test('A upgrade: real P24 committed intent becomes discoverable without a new business write and retains its identity/payload',async()=>{
 ...
 const files=(await readdir(new URL('../../../../db/migrations/',import.meta.url))).filter(n=>n.endsWith('.sql')&&!n.startsWith('0024_'));
 const app=Fastify();
 try{
  ...
  expect(before).toHaveLength(1);expect(await migrate(old.pool)).toEqual(['0024_outbox_delivery.sql']);
