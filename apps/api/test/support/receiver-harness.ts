...
export async function receiverWorker(database:string,config:ReceiverConfig,options:{once?:boolean;entry?:string;cwd?:string}={}){
 ...
 const child=spawn(process.execPath,[...(options.entry?[]:['--import','tsx']),options.entry??'apps/mock-erp/src/main.ts','worker',...(options.once?['--once']:[])],{windowsHide:true,stdio:['ignore','pipe','pipe'],env,...(options.cwd?{cwd:options.cwd}:{})});
