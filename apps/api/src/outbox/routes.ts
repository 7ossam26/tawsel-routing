...
export async function outboxRoutes(app:FastifyInstance,pool:Pool,config?:OutboxConfig){
 const reads=new OutboxReads(pool),service=new OutboxService(pool,config),recovery=new ReconciliationReads(pool);
 ...
 for(const op of ['integration.configureWebhook','integration.rotateSigningKey','integration.retryDelivery','integration.reportAppliedCheckpoint'])app.post(`/api/v1/integration/commands/${op}`,{bodyLimit:16384},async(request,reply)=>{
 ...
 for(const [path,read] of [['reconciliation',recovery.snapshot.bind(recovery)],['applied-checkpoint',recovery.report.bind(recovery)]] as const)app.get(`/api/v1/integration/${path}`,async r=>{
  const q=query(r.query,['aggregateType','aggregateId']);if(!['task','assignment','trip','workday','return-request','integration'].includes(String(q.aggregateType)))throw invalid();return read(r.headers.authorization,String(q.aggregateType),uuid(q.aggregateId));
 });
}
