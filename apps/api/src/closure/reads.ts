...
async function carried(tx:Transaction,a:AccessSession,d:DayRow,asOf:string):Promise<CarryForward>{
 ...
 const histories=(await tx.query<{record:Outcome}>('SELECT record FROM tawsel.delivery_outcomes o WHERE NOT EXISTS (SELECT 1 FROM tawsel.delivery_outcomes newer WHERE newer.tenant_id=o.tenant_id AND newer.attempt_id=o.attempt_id AND newer.revision>o.revision) AND tenant_id=$1 AND task_id=ANY($2::uuid[]) ORDER BY revision',[d.tenant_id,ids])).rows.map(r=>r.record);
...
export class WorkdayReads {
 ...
 async summary(principal:AuthenticatedPrincipal,workdayId:string):Promise<Summary>{
  uuid(workdayId);return withAccess(this.pool,principal,async(a,tx)=>{
   await lockInvariants(tx,a.context.tenantId,[{kind:'driver',id:ownDriver(a)}]);await authorize(tx,a,workdayId);const d=await day(tx,a,workdayId),now=(await tx.query<{now:Date}>('SELECT clock_timestamp() AS now')).rows[0]!.now;
   const rounds=(await tx.query<{round_id:string;started_at:Date;ended_at:Date|null;first_plan_id:string;first_forecast_id:string;first_workload_id:string;activity_revision:string}>('SELECT r.*,COALESCE(a.revision,0) AS activity_revision FROM tawsel.rounds r LEFT JOIN tawsel.round_activity_state a USING(tenant_id,round_id) WHERE r.tenant_id=$1 AND workday_id=$2 ORDER BY device_generation',[d.tenant_id,workdayId])).rows.map(r=>({roundId:r.round_id,activityRevision:Number(r.activity_revision),startedAt:r.started_at.toISOString(),endedAt:r.ended_at?.toISOString()??null,firstPlanId:r.first_plan_id,firstForecastId:r.first_forecast_id,firstWorkloadId:r.first_workload_id}));
   ...
   const outcomes=(await tx.query<{record:Outcome}>(`SELECT o.record FROM tawsel.delivery_outcomes o JOIN tawsel.rounds r USING(tenant_id,round_id) WHERE NOT EXISTS (SELECT 1 FROM tawsel.delivery_outcomes newer WHERE newer.tenant_id=o.tenant_id AND newer.attempt_id=o.attempt_id AND newer.revision>o.revision) AND r.tenant_id=$1 AND r.workday_id=$2 ORDER BY o.record->'time'->>'recordedAt',o.revision,o.outcome_id`,[d.tenant_id,workdayId])).rows.map(r=>r.record);
