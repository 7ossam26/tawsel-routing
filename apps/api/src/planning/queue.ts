...
interface TaskRow {
 ...
 priority:'ordinary'|'urgent'|null; reservation_state:string|null; attempt_id:string;
}
export async function snapshot(tx:Transaction,state:StateRow):Promise<Input> {
 ...
 const rows=(await tx.query<TaskRow>(`SELECT t.*,l.revision AS location_revision,l.source_revision AS pin_source_revision,l.latitude,l.longitude,
  c.assignment_revision,s.payload->>'priority' AS priority,p.state AS reservation_state,a.attempt_id
 ...
 const members:Member[]=rows.map(r=>{
  ...
  const exclusionReason:Member['exclusionReason']=r.kind==='company'&&r.state!=='held'?'not-held':!coordinates?'location-unresolved':
