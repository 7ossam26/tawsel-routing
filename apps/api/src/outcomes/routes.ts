...
import { Outcomes } from './service.js';
import { OutcomeError,requireOutcome } from './models.js';
export async function outcomeRoutes(app:FastifyInstance,pool:Pool,config:AuthConfig,authenticate?:PlanningAuthenticator,service=new Outcomes(pool)){
 ...
 for(const [path,name] of [['full','FullCommand'],['partial','PartialCommand'],['refusal','RefusalCommand'],['no-answer','NoAnswerCommand']])app.post(`/api/v1/outcomes/${path}`,{schema:{querystring:query}},async(request,reply)=>{
  requireBrowserCsrf(request,config);requireOutcome(name!,request.body);const {kind}=request.query as {kind:'personal'|'company'};
  const result=await use(request,kind,principal=>service.command(principal,request.body));requireOutcome('ActionResult',result);return reply.status(result.response?.status??202).send(result);
 });
 app.get('/api/v1/outcomes/rounds/:roundId',{schema:{querystring:query}},async request=>{const {kind}=request.query as {kind:'personal'|'company'},{roundId}=request.params as {roundId:string};return use(request,kind,principal=>service.read(principal,roundId));});
 app.get('/api/v1/outcomes/actions/:actionId',{schema:{querystring:query}},async(request,reply)=>{const {kind}=request.query as {kind:'personal'|'company'},{actionId}=request.params as {actionId:string};const result=await use(request,kind,principal=>service.result(principal,actionId));return reply.status(result.status==='pending'?202:200).send(result);});
}
