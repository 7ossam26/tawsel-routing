...
export async function receiverProcess(databaseUrl:string,config:ReceiverConfig,fault?:string,options:{entry?:string;cwd?:string}={}){
 ...
 const url=await new Promise<string>((resolve,reject)=>{
  const timeout=setTimeout(()=>reject(new Error(`Receiver startup timeout ${errors}`)),15000);
