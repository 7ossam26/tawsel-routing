...
function check(source:string,file:string){
 ...
 function module(value:ts.Expression){
  ...
  }else if(!name.startsWith('node:')&&!['pg','fastify','@tawsel/api-client'].includes(name)&&!name.startsWith('@tawsel/api-client/'))throw new Error('Consumer internal module dependency');
 }
...
test('external consumer runtime and dependency manifest cannot import Tawsel internals; detector rejects escape and dynamic bypass fixtures',()=>{
 ...
 expect(Object.keys(pkg.dependencies).sort()).toEqual(['@tawsel/api-client','fastify','pg']);
