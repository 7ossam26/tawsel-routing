...
export function ContactActions({ phone = '+20 10 0000 0000',coordinates,onExternalOpen }: { phone?: string;coordinates?:{latitude:number;longitude:number};onExternalOpen?:(label:string)=>void }) {
  const normalized = phone.replaceAll(' ', '');
  return <div className="contact-actions" aria-label="التواصل والاتجاه"><a href={`tel:${normalized}`} aria-label={`اتصال بـ ${phone}`} onClick={()=>onExternalOpen?.('الاتصال')}><Phone aria-hidden="true" /><span>اتصال</span></a><a href={`https://wa.me/${normalized.replace('+', '')}`} target="_blank" rel="noreferrer" aria-label={`واتساب مع ${phone}`} onClick={()=>onExternalOpen?.('واتساب')}><MessageCircle aria-hidden="true" /><span>واتساب</span></a>{coordinates?<a href={`https://www.google.com/maps/dir/?api=1&destination=${coordinates.latitude},${coordinates.longitude}`} target="_blank" rel="noreferrer" aria-label="الاتجاهات إلى موقع العميل" onClick={()=>onExternalOpen?.('الاتجاهات')}><Navigation aria-hidden="true" /><span>الاتجاهات</span></a>:<button type="button" disabled aria-label="الاتجاهات غير متاحة"><Navigation aria-hidden="true" /><span>الاتجاهات</span></button>}</div>;
}
