...
type OutcomeCommand = components['schemas']['OutcomeFullCommand'] | components['schemas']['OutcomeNoAnswerCommand'];
...
export function CurrentActivityPage() {
  ...
  const execute = useCallback(async (item: Pending) => {
    ...
    try {
      ...
      if (item.kind === 'current') {
        ...
        if (result.receipt.businessStatus !== 'accepted') throw new Error(result.receipt.problem?.detail ?? 'لم يُقبل الإجراء.');
      } else if (item.kind === 'outcome') {
        const result = item.command.operationId === 'outcome.recordFull' ? await outcomeClient.full(item.command) : await outcomeClient.noAnswer(item.command);
        if (result.receipt.businessStatus !== 'accepted') throw new Error(result.receipt.problem?.detail ?? 'لم تُقبل النتيجة.');
        setSuccess(item.command.operationId === 'outcome.recordFull' ? 'تم تأكيد التسليم والتحصيل من الخادم. المحطة التالية اقتراح فقط.' : 'تم تسجيل عدم الرد من الخادم دون وصول أو رسوم أو عدّاد مكالمات.');
      } else {
        const result = await devicesClient.continueOnThisPhone(item.command);
        if (result.result.receipt.businessStatus !== 'accepted' || result.snapshot?.context.mode !== 'owner') throw new Error(result.result.receipt.problem?.detail ?? 'لم يكتمل نقل التنفيذ وتحميل الحالة.');
      ...
      discardPending(); await refresh(); headingRef.current?.focus();
    } catch (failure) {
      if (failure instanceof Error && !/تعذر الاتصال|Failed to fetch|network/i.test(failure.message)) { discardPending(); setError(failure.message); }
  ...
  return <main className="tasks-shell current-shell" dir="rtl"><header className="tasks-header"><div><p className="eyebrow">توصيل · جولتك</p><h1 ref={headingRef} tabIndex={-1}>المحطة الحالية</h1><p>{current?.stage === 'arrived' ? 'الوصول مسجّل؛ أكّد التسليم والتحصيل مرة واحدة.' : current ? 'أكّد الوصول عند العميل، أو سجّل عدم الرد دون اختلاق وصول.' : 'اختر المحطة ثم ابدأ الاتجاه إليها صراحةً.'}</p></div><MapPin aria-hidden="true" /></header>
    ...
    {!loaded ? <StatusNotice title="جارٍ تحميل الجولة" /> : !state ? (!error ? <StatusNotice title="لا توجد جولة نشطة">ابدأ جولة متزامنة أولًا من تجهيز العمل.</StatusNotice> : null) : <>
      ...
      <ActiveRouteMap targets={state.targets} selectedTaskId={selected} currentAttemptId={current?.attemptId} onSelect={selectTarget} />
