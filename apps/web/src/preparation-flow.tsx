...
export function PreparationFlow() {
  ...
  function coordinates(latitude: string, longitude: string) {
    if (!latitude.trim() || !longitude.trim()) throw new Error('أكمل إحداثيات النقطة المطلوبة قبل تجهيز الخطة.');
  ...
  async function submitPlanning(command: PlanningCommand) {
    ...
    try {
      ...
      setNotice(command.operationId === 'planning.setManualOrder' ? 'اعتمد الخادم الترتيب اليدوي. يمكنك مراجعته ثم بدء الجولة.' : 'حُفظت اختياراتك. راجع حالة الخطة أدناه.');
      await load();
