...
import { loadLocalDraft, saveLocalDraft } from './local-drafts';
...
export function ExceptionEditor({ mode, delivery, draftKey, disabled, busy, error, pending, onClose, onSave }: { mode: 'partial' | 'refused'; delivery: Delivery; draftKey: string; disabled: boolean; busy: boolean; error: string; pending: boolean; onClose: () => void; onSave: (result: Replacement) => void }) {
  ...
  const touched = useRef(false), [hydrated, setHydrated] = useState(false);
  useEffect(() => {
    let active = true;
    void loadLocalDraft<ResultDraft>(draftKey).then(saved => { if (active) { if (saved && !touched.current) setDraft(saved); setHydrated(true); } }).catch(() => { if (active) setDraftError('تعذر قراءة المسودة المحفوظة. احتفظ بالمدخلات وحاول مجددًا.'); });
    return () => { active = false; };
  }, [draftKey]);
  useEffect(() => {
    try { sessionStorage.setItem(draftKey, JSON.stringify(draft)); } catch { setDraftError('تعذر الاحتفاظ بالمسودة بعد الإغلاق؛ المدخلات ما زالت أمامك.'); }
    if (hydrated) void saveLocalDraft(draftKey, draft).catch(() => setDraftError('تعذر حفظ المسودة على الهاتف. المدخلات ما زالت أمامك.'));
