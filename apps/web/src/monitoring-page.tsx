...
export function MonitoringPage() {
  ...
  return <main className="monitor-shell" data-monitoring-revision={state.revision} data-current-task={data?.current?.taskId??undefined}><header className="monitor-header"><div><p className="eyebrow">مساحة المتابعة المشتركة</p><h1>حالة المندوب والعمل</h1><p>حالة مؤكدة من الخادم، بلا تتبع موقع أو افتراض أن الهاتف متصل.</p></div><a href={`/account?kind=${kind}`}>الحساب</a></header>
    {context?.access.effectiveCapabilities.includes('reports.read') ? <a className="edit-link" href={`/reports?kind=${kind}${driverId?`&driverId=${encodeURIComponent(driverId)}`:''}`}>تقارير أيام العمل والتوقيت</a> : null}
