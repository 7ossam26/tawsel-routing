...
export async function withJournalLock<T>(scope: string, work: () => Promise<T>): Promise<T> {
  ...
  return navigator.locks.request('tawsel:lifecycle', { mode: 'shared', signal: AbortSignal.timeout(30000) }, () =>
    navigator.locks.request('tawsel:journal:' + scope, { signal: AbortSignal.timeout(30000) }, work));
}
