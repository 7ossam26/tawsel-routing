...
describe('canonical public contract foundation (schema evidence, not business execution)', () => {
  ...
  it('does not publish designed operations as available HTTP paths', () => {
    for (const path of Object.keys(bundle.api.paths)) expect(path).toMatch(/^\/api\/(session\/|account\/status$|v1\/(?:provisioning\/|independent\/tasks|intake\/|locations|maps\/|planning\/|rounds\/|current\/|outcomes\/|eligibility\/|closure\/|workdays\/|devices\/|actions\/|evidence\/|routing\/profiles$))/);
  ...
  it('rejects missing owners, omitted families, duplicate IDs and invented public availability', () => {
    ...
    invented.api.paths = { '/test-only-mutation': { post: { operationId: 'evidence.adoptCompatible' } } };
