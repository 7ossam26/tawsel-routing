...
describe('canonical public contract foundation (schema evidence, not business execution)', () => {
  ...
  it('does not publish designed operations as available HTTP paths', () => {
    for (const path of Object.keys(bundle.api.paths)) expect(path).toMatch(/^\/api\/(session\/|account\/status$|v1\/(?:provisioning\/|integration\/|consumer\/|independent\/tasks|intake\/|locations|maps\/|planning\/|rounds\/|current\/|outcomes\/|corrections\/|eligibility\/|closure\/|workdays\/|devices\/|actions\/|evidence\/|returns\/|branches\/|erp\/(?:returns|monitoring)\/|monitoring\/|routing\/profiles$))/);
  ...
  it('rejects missing owners, omitted families, duplicate IDs and invented public availability', () => {
    ...
    invented.api.paths = { '/test-only-mutation': { post: { operationId: 'integration.applicationReported' } } };
  ...
  it('generates identical portable client/reference bytes and detects checked-in drift', async () => {
    ...
    expect(first.get('docs/reference/public-contract.md')).toContain('P26 adds independent durable receipt/projection, received/applied reports and scoped replay/checkpoint recovery with honest history gaps.');
  });
});
